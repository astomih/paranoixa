...
Vector2 Vector2::transform(const Vector2 &vec, const class Matrix3 &mat,
                           float w) {
...
Quaternion Quaternion::fromEuler(const Vector3 &euler) {
  ...
  q = Quaternion::concatenate(
      q, Quaternion(Vector3::unitZ, Math::toRadians(euler.z)));
  q = Quaternion::concatenate(
      q, Quaternion(Vector3::unitY, Math::toRadians(euler.y)));
  q = Quaternion::concatenate(
      q, Quaternion(Vector3::unitX, Math::toRadians(euler.x)));
  return q;
}
